...
func TestRepository_PersistClaimAfterAdjudication_DoesNotRecheckCampaignState(t *testing.T) {
	db := openIntegrationDB(t)
	ctx := context.Background()
	couponID := nextTestCouponID()
	resetTestData(t, db, couponID)
...
`, couponID); err != nil {
	...
	rec, err := repo.PersistClaimAfterAdjudication(ctx, couponID, 24001, "redis-admitted")
	if err != nil {
		t.Fatalf("persist after adjudication should not recheck campaign state, got %v", err)
	}
	if rec.ID <= 0 {
		t.Fatalf("expected persisted claim id, got %d", rec.ID)
	}
	if got := countClaimsByUser(t, db, couponID, 24001); got != 1 {
		t.Fatalf("expected one persisted claim record, got %d", got)
	}
	if got := loadCampaignStock(t, db, couponID); got != 4 {
		t.Fatalf("expected available stock 4 after persistence, got %d", got)
	}
}
