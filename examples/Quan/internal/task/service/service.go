...
func NewService(txm *mysql.TxManager, repo *taskrepo.Repository, outboxRepo *outbox.Repository, defaultRetry int) *Service {
	return NewServiceWithQueue(txm, repo, outboxRepo, nil, defaultRetry)
}

func NewServiceWithQueue(txm *mysql.TxManager, repo *taskrepo.Repository, outboxRepo *outbox.Repository, queue dlqQueue, defaultRetry int) *Service {
	if defaultRetry <= 0 {
		defaultRetry = 5
	}
	return &Service{
...
func (s *Service) CreateTask(ctx context.Context, req CreateTaskRequest) (taskmodel.AsyncTask, error) {
	...
	if req.TaskType == "" || req.BizID == "" || len(req.Payload) == 0 {
		return taskmodel.AsyncTask{}, apperr.New(apperr.CodeBadRequest, "invalid task request")
	}
	if req.MaxRetry <= 0 {
		req.MaxRetry = s.defaultRetry
	}
	var created taskmodel.AsyncTask
	err := s.txm.WithinTx(ctx, nil, func(ctx context.Context, tx *sql.Tx) error {
		task, err := s.repo.CreateTx(ctx, tx, taskmodel.CreateTaskParams{
		...
		if err != nil {
			if errors.Is(err, taskrepo.ErrTaskDuplicate) {
	...
	if err != nil {
		return taskmodel.AsyncTask{}, apperr.Wrap(apperr.CodeInternalError, "create task failed", err)
	}
	return created, nil
}

func (s *Service) GetTask(ctx context.Context, taskID int64) (taskmodel.AsyncTask, error) {
	task, err := s.repo.GetByID(ctx, taskID)
	if err != nil {
		if errors.Is(err, sql.ErrNoRows) || errors.Is(err, taskrepo.ErrTaskNotFound) {
			return taskmodel.AsyncTask{}, apperr.New(apperr.CodeNotFound, "task not found")
		}
		return taskmodel.AsyncTask{}, apperr.Wrap(apperr.CodeInternalError, "query task failed", err)
	}
	return task, nil
}
...
func (s *Service) ReplayDLQTask(ctx context.Context, taskID int64) error {
	...
	if err := s.repo.MarkReplayReady(ctx, taskID); err != nil {
		...
		case errors.Is(err, taskrepo.ErrTaskNotReplayable):
